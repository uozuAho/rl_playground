...
public class ValueNetworkTrainer
{
    ...
    public static void TrainAndTestValueNetwork(string datasetPath)
    {
        ...
        var batchRate =
            numEpochs * trainPositions.Count / (double)batchSize / stopwatch.Elapsed.TotalSeconds;
        ...
        Console.WriteLine(
            $"Training took {stopwatch.Elapsed}\n"
        ...
        using (no_grad())
        {
            using var testPositionsT = stack(testPositions.Select(Board2Tensor).ToArray())
                .to(device);
