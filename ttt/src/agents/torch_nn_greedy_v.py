...
class ConvNet(nn.Module):
    ...
    def learn_batch(self, batch: t.List[GameStep]):
        ...
        non_final_next_states = torch.stack(
            [self.state2input(b.next_state) for b in batch if not b.is_end]
        ).to(self.device)
        ...
        if any(b.is_end for b in batch):
            final_states = torch.stack(
                [self.state2input(b.next_state) for b in batch if b.is_end]
            ).to(self.device)
            rewards = (
                torch.tensor([b.reward for b in batch if b.is_end], device=self.device)
                .unsqueeze(0)
                .t()
            )
...
class NnGreedyVAgent(TttAgent):
    ...
    def get_action(self, env: t3.TttEnv, epsilon=0.0):
    ...
    def state_val(self, env: t3.TttEnv):
    ...
    def action_vals(self, env: t3.TttEnv):
        """For debugging"""
        vals = {}
    ...
    def _greedy_action(self, env: t3.TttEnv):
