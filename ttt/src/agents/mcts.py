...
# player.
type ValFunc = t.Callable[[t3.TttEnv, t3.Player], float]


def random_rollout_reward(env: t3.TttEnv, player: t3.Player):
...
class MctsAgent(TttAgent):
    ...
    def get_action(self, env: t3.TttEnv):
        return mcts_decision(env, self.n_sims, self._valfn, self._use_valfn_for_expand)

    def get_actions(self, envs: list[t3.TttEnv]) -> list[int]:
        return [self.get_action(e) for e in envs]

    def print_tree(self, env: t3.TttEnv, n_sims=-1):
        """For debugging"""
        n_sims = n_sims if n_sims > 0 else self.n_sims
        tree = build_mcts_tree(env, n_sims, self._valfn, self._use_valfn_for_expand)
        print_tree(tree)
...
def mcts_decision(
    ...
    root = build_mcts_tree(env, n_simulations, val_func, use_val_func_for_expand)
    best_move = max(
        root.children,
...
class _MCTSNode:
    def __init__(self, state: t3.TttEnv, parent, val_est: float | None = None):
        self.state = state
    ...
    def __str__(self):
        return f"{self.state.str1d_sep('|')} vis{self.visits:3} tval{self.total_reward:5.2f} uct{self.ucb1():5.2f}"

    def __repr__(self):
        board = self.state.str1d_sep("|")
        return f"{board} vis {self.visits:3} u: {self.ucb1():.2f} tv: {self.total_reward:5.2f} v_est: {self.val_est}"
...
def build_mcts_tree(
    env: t3.TttEnv, simulations: int, val_func: ValFunc, use_val_func_for_expand
):
