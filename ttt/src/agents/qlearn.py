...
from agents.sarsa import QaTable
...
class TabQlearnAgent(TttAgent):
    ...
    def __init__(self, q_table: QaTable | None = None, allow_invalid_actions=False):
    ...
    def train_new(opponent: TttAgent, n_eps: int):
        ...
        agent.train(t3.TttEnv(), opponent, n_eps)
        return agent
        ...
        for episode in range(n_training_episodes):
            ...
            while not done:
                ...
                if env.current_player == t3.X:
                    action = egreedy_policy(
                        env, self._q_table, epsilon, self.allow_invalid_actions
                    )
                    _, reward, terminated, truncated, _ = env.step(action)
                    new_state = env
                    done = terminated or truncated
                    ...
                    maxq = max(
                        [self._q_table.value(envstate(new_state), a) for a in range(9)],
                        default=0,
                    )
                    current_q = self._q_table.value(envstate(state), action)
                    new_value = current_q + learning_rate * (
                        reward + gamma * maxq - current_q
                    )
...
def greedy_policy(env: t3.TttEnv, qtable: QaTable, allow_invalid):
...
def egreedy_policy(env: t3.TttEnv, qtable: QaTable, epsilon: float, allow_invalid):
