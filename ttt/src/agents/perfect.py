...
def player_won(env: t3.TttEnv, player: t3.Player):
    return t3.winner(env.board) == player


def find_best_move(env: t3.TttEnv, player=t3.X):
    ...
    if player == t3.O and num_empty == 6:
        ...
        if (env.board[0] == x and env.board[8] == x) or (
