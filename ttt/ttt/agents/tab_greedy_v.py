...
class TabGreedyVAgent(TttAgent):
    ...
    def train_new(n_eps: int):
        agent = TabGreedyVAgent()
        agent.train(t3.GymEnv(), n_eps)
        return agent

    def get_action(self, env: t3.GymEnv):
        return greedy_policy(env, self._q_table)

    def action_values(self, board_str: str):
        """ For debugging """
        env = t3.GymEnv.from_str(board_str)
        values = {}
...
def greedy_policy(env: t3.GymEnv, qtable: Qtable):
...
def egreedy_policy(env: t3.GymEnv, qtable: Qtable, epsilon: float):
